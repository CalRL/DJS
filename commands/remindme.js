...
module.exports = {
    ...
    var { hours, hour, hrs, h } = Math.floor((distance % (1000 * 60 * 60 * 24)) / (1000 * 60 * 60));
    var { minutes, minute, mins, m } = Math.floor((distance % (1000 * 60 * 60)) / (1000 * 60));
    ...
    if(!time || !unit) {
        ...
        if(distance < 0)
        message.channel(userId).send("Timer is up!")
        })
    }
